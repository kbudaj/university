...
def parzysta(n):
   return n%2 != 1 # Ups, wszystkie liczby sa parzyste [!]
...
def sumaParzystych(L): #[!]
   ...
   wynik = 0
   for element in filter(parzysta, L):
       wynik += element
   return wynik
...
def histogram(L):
   ...
   for liczba in L:
      print (str(liczba)+ " " * (3-int(len(str(liczba)))) + "*" * liczba) #[!]
...
def madrzejsza(L): #[!]
   ...
   return [x + " " + random.choice(madreSlowa) for x in L if len(str(x)) >= 3]
